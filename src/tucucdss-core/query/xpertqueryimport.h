...
public:
    ...
    [[nodiscard]] Status importFromFile(std::unique_ptr<XpertQueryData>& _query, const std::string& _fileName);

    /// \brief Import a query based on an XML string. This function is reentrant.
    /// \param _query A reference to an xpert query pointer that will be allocated in the function.
    /// \param _xml An XML string that represents the query.
    /// \return Status::Ok if the import went well, otherwise another Status.
    [[nodiscard]] Status importFromString(std::unique_ptr<XpertQueryData>& _query, const std::string& _xml);
...
protected:
    ...
    /// \return Status::Ok if the import went well, otherwise another Status.
    [[nodiscard]] Status importDocument(std::unique_ptr<XpertQueryData>& _query, Common::XmlDocument& _document);
    ...
    [[nodiscard]] std::unique_ptr<AdminData> createAdminData(Common::XmlDocument& _document);
    ...
    [[nodiscard]] std::unique_ptr<FullPersonData> createFullPersonData(Common::XmlNode& _root, bool _isPatient);
    ...
    [[nodiscard]] std::unique_ptr<PersonData> createPersonData(
            Common::XmlNodeIterator& _personDataRootIterator,
            const std::optional<std::string>& _birthdate = std::nullopt);
    /// \brief Create an empty person data object.
    /// \return A unique pointer on the empty person data.
    [[nodiscard]] std::unique_ptr<PersonData> createEmptyPersonData();

    [[nodiscard]] std::optional<std::string> extractBirthdate(Common::XmlNode& _root);
    ...
    /// it returns an empty data object.
    [[nodiscard]] std::unique_ptr<InstituteData> createInstituteData(Common::XmlNodeIterator& _instituteRootIterator);
    /// \brief Create an empty institute data object.
    /// \return A unique pointer on the empty institute data
    [[nodiscard]] std::unique_ptr<InstituteData> createEmptyInstituteData();
    ...
    /// it returns an empty data object.
    [[nodiscard]] std::unique_ptr<AddressData> createAddressData(Common::XmlNodeIterator& _addressRootIterator);
    /// \brief Create an empty address data object.
    /// \return A unique pointer on the empty address data.
    [[nodiscard]] std::unique_ptr<AddressData> createEmptyAddressData();
    ...
    /// it returns an empty data object.
    [[nodiscard]] std::unique_ptr<PhoneData> createPhoneData(Common::XmlNodeIterator& _phoneRootIterator);
    /// \brief Creates an empty phone data object.
    /// \return A unique pointer on the empty phone data.
    [[nodiscard]] std::unique_ptr<PhoneData> createEmptyPhoneData();
    ...
    /// it returns an empty data object.
    [[nodiscard]] std::unique_ptr<EmailData> createEmailData(Common::XmlNodeIterator& _emailRootIterator);
    /// \brief Creates an empty email data object.
    /// \return A unique pointer on the empty email data.
    [[nodiscard]] std::unique_ptr<EmailData> createEmptyEmailData();
    ...
    [[nodiscard]] std::unique_ptr<ClinicalDatas> createClinicalDatas(
            Common::XmlNodeIterator& _clinicalDatasRootIterator);
    ...
    [[nodiscard]] std::unique_ptr<XpertRequestData> createXpertRequestData(
    ...
    [[nodiscard]] std::string getChildStringOptional(
    ...
    [[nodiscard]] LoadingOption getChildLoadingOptionEnumOptional(
    ...
    [[nodiscard]] RestPeriodOption getChildRestPeriodOptionEnumOptional(
            Common::XmlNodeIterator _xpertRequestRootIterator,
    ...
    [[nodiscard]] Core::TargetExtractionOption getChildTargetExtractionOptionEnumOptional(
            Common::XmlNodeIterator _xpertRequestRootIterator,
            const std::string& _childName,
    ...
    [[nodiscard]] Core::FormulationAndRouteSelectionOption getChildFormulationAndRouteSelectionOptionEnumOptional(
            Common::XmlNodeIterator _xpertRequestRootIterator,
            const std::string& _childName,
    ...
    [[nodiscard]] Common::DateTime getChildDateTime(
            Common::XmlNodeIterator _rootIterator, const std::string& _childName);

private:
    Tucuxi::Common::LoggerHelper m_logHelper;
};

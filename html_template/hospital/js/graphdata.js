...
class GraphPredictionResult {
    constructor() {
        ...
        this.X = this.predictive.predictionData.times();
        this.Y = this.predictive.predictionData.values();
    }
}
...
const conversionMap = {
    Weight: {
        "kg": 1000.0, "Kg": 1000.0, "g": 1.0, "mg": 0.001, "ug": 0.000001, "µg": 0.000001, "ng": 0.000000001
    },
    MolarMass: {
        "g/mol": 1.0, "g/umol": 1000000.0, "kg/mol": 1000.0, "kg/umol": 1000000000.0,
        "mg/mol": 0.001, "ug/mol": 0.000001, "µg/mol": 0.000001, "mg/umol": 1000.0, "ug/umol": 1.0, "µg/umol": 1.0
    },
    Concentration: {
        "g/l": 1.0, "mg/l": 0.001, "mg/dl": 0.01, "ug/l": 0.000001, "g/ml": 1000, "mg/ml": 1.0, "ug/ml": 0.001,
        "µg/l": 0.000001, "ng/l": 0.000000001, "g/dl": 10.0, "µg/dl": 0.00001, "ug/dl": 0.00001, "ng/dl": 0.00000001,
        "mg/cl": 0.1, "µg/cl": 0.0001, "ug/cl": 0.0001, "ng/cl": 0.0000001, "µg/ml": 0.001, "ng/ml": 0.000001
    },
    Time: {
        "min": 1.0, "s": 1 / 60.0, "h": 60.0, "d": 24.0 * 60.0, "w": 7.0 * 24.0 * 60.0,
        "week": 7.0 * 24.0 * 60.0, "month": 30.0 * 24.0 * 60.0, "y": 365.0 * 24.0 * 60.0
    },
    NoUnit: {
        "-": 1.0, "": 1.0
    }
...
class GraphAmount {
    ...
    static getConversionFactor(unit) {
        for (const category in conversionMap) {
            if (conversionMap[category][unit] !== undefined) {
                return conversionMap[category][unit];
            }
        }
    ...
    valueInUnit(unit) {
        const factor = GraphAmount.getConversionFactor(unit);
        return factor !== null ? this.dbvalue * factor * this.multiplier : null;
    }
}

// This class represents a value in conjunction with a multiplier coefficient and a unit.
// For the web front end, the multiplier could stay at 1.
class GraphIdentifiableAmount extends GraphAmount {
...
// This class represents a value in conjunction with a multiplier coefficient and a unit.
// For the web front end, the multiplier could stay at 1.
class GraphIdentifiableAmountUnit {
    ...
    valueInUnit(unit) {
        const factor = GraphAmount.getConversionFactor(unit);
        const thisFactor = GraphAmount.getConversionFactor(this.unitstring);
        return factor !== null ? 
        	(thisFactor !== null ? this.dbvalue * factor /thisFactor : null) : null;
    }
}
...
class GraphMeasure {
    constructor(moment, value, multiplier, unit) {
        this.moment = moment;
        this.concentration = new GraphIdentifiableAmountUnit(value, unit);
        this.enable = 1;
    }
}
